...
Regardless of how you use Shiki, the more code you highlight, the more likely it is that you will run into performance issues. After testing, our team concluded that **creating Highlighter instances and loading languages are Shiki's the most resource-intensive operations**.
...
export async function getHighlighter({ theme, langs }) {
...
This simple caching solution made our build times a remarkable **80% faster**. We tried various other suggestions, none worked as effectively as creating a caching system for Shiki Highlighter instances.

Here are a couple of things to keep in mind:

- The keys used to identify instances is prone to duplicates if Highlighter instances are created using different aliases, i.e. `typescript` and `ts`. Check the Shiki's [language aliases](https://github.com/shikijs/shiki/blob/main/docs/languages.md#all-languages) and make sure your team has a convention for their use.
